...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.setTimeout(function () {
    document
      .getElementById("recordCount")
      .setAttribute("class", "btn btn-info form-control px-5");
  }, 2000);
});
...
function addRecord(name, salary) {
  ...
  const myArray = Object.keys(salary_data);
  const lastKey = myArray[myArray.length - 1];
  const id = parseInt(lastKey) + 1;
